...
#include <QDebug>
cloudAmountProcessing::cloudAmountProcessing(QString& monthPath)
{
    minStandardX = 150;
    maxStandardX = 0;
    maxYBoundary = 0;
...
//一个线程处理一个月的数据
void cloudAmountProcessing::process()
{
...
void cloudAmountProcessing::run()
{
    qDebug()<<"现在线程ID是："<<QThread::currentThreadId();
    process();
}
